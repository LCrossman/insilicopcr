...
impl Fairing for CORS {
  ...
  async fn on_response<'r>(&self, request: &'r Request<'_>, response: &mut Response<'r>) {
    ...
    let allowed_origins = "https://insilicopcr.shuttleapp.rs";
...
const HOST: Absolute<'static> = uri!("https://insilicopcr.shuttleapp.rs");
...
impl<'r> RocketGovernable<'r> for RateLimitGuard {
    fn quota(_method: in_silico_pcr_rocket_governor::Method, _route_name: &str) -> Quota {
...
async fn rocket() -> shuttle_rocket::ShuttleRocket {
    let rocket = rocket::build()
       .attach(CORS)
       .mount("/", routes![serve, upload, retrieve, route_example])
